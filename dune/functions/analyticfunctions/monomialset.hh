...
namespace Dune::Functions {
  ...
  namespace Impl {
    ...
    // Computes y = [1, x, x^2, x^3, ..., x^degree] with x a single coordinate
    template<int degree, class DomainFieldType, class RangeType>
    void computePowers(const DomainFieldType& x, RangeType& y)
    {
      if constexpr(degree >= 0)
      {
        y[0] = 1;
        for(auto k : Dune::range(degree))
          y[k+1] = y[k]*x;
      }
    }
  ...
  /**
   * \brief Set of all monomials up to degree \p degree as vector valued functions.
   *
   * \ingroup FunctionImplementations
   *
   * \tparam RangeFieldType scalar type.
   * \tparam dim Domain dimension.
   * \tparam degree Maximal monomial order.
   *
   * This vector valued function contains the dim-variate monomials up to
   * order \p degree as components and models the
   ...
   * This is currently specialized for \p dim = 1, \p dim = 2, and \p dim = 3 only.
   */
  template<class RangeFieldType, int dimension, int degree>
  struct MonomialSet{
    static constexpr int dim = dimension;
    static constexpr int size = Dune::binomial(degree + dim, dim);
    ...
    */
    ...
    /**
    * \brief Set of all first order derivatives of monomials up to degree \p degree as vector of vector valued functions.
    */
    struct Derivative
    {
      /**
  ...
  template<class RangeFieldType, int degree>
  struct MonomialSet<RangeFieldType, 1, degree>
  {
    static constexpr int dim = 1;
    static constexpr int size = degree+1;
    ...
    constexpr auto operator()(const Dune::FieldVector<DomainFieldType,1>& x) const
    {
      ...
      Impl::computePowers<degree>(x[0], y);
      return y;
    }
    ...
    struct Derivative
    {
      ...
      constexpr auto operator()(const Dune::FieldVector<DomainFieldType,1>& x) const
      {
        ...
        Impl::computePowers<degree-1>(x[0], xPowers);

        auto y = Dune::FieldMatrix<RangeFieldType,size,1>{};
        for(auto order : Dune::range(1, degree+1))
  ...
  template<class RangeFieldType, int degree>
  struct MonomialSet<RangeFieldType, 2, degree>
  {
    static constexpr int dim = 2;
    static constexpr int size = (degree+1)*(degree+2)/2;
    ...
    struct Derivative
    {
      ...
      constexpr auto operator()(const Dune::FieldVector<DomainFieldType,2>& x) const
      {
        auto xPowers = std::array<Dune::FieldVector<RangeFieldType,size>,dim>{};
        for(auto j : Dune::range(dim))
          Impl::computePowers<degree-1>(x[j], xPowers[j]);
      ...
      struct Hessian
      {
        ...
        constexpr auto operator()(const Dune::FieldVector<DomainFieldType,2>& x) const
        {
          auto xPowers = std::array<std::array<RangeFieldType,degree+1>,dim>{};
          for(auto j : Dune::range(dim))
  ...
  template<class RangeFieldType, int degree>
  struct MonomialSet<RangeFieldType, 3, degree>
  {
