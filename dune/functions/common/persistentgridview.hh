...
#ifndef DUNE_FUNCTIONS_COMMON_PERSISTENTGRIDVIEW_HH
#define DUNE_FUNCTIONS_COMMON_PERSISTENTGRIDVIEW_HH
...
namespace Dune::Functions::Experimental {
   ...
   * the underlying grid view during creation in a persistent
   * way. The cache is filled lazily:
   *
   * After creation any accessed index is obtained from
   * the index set of the underlying grid view and then stored
   ...
   * touching the grid view used for filling the cache.
   *
   * While the PersistentIndexSet can be used after grid modification,
   * this requires that entities passed to the index methods
   * entities still exist within the grid.
   */
  template<class GV>
...
#endif// DUNE_FUNCTIONS_COMMON_PERSISTENTGRIDVIEW_HH
