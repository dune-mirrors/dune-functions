...
namespace Dune::Functions
{
  ...
  namespace Impl
  {
    ...
    /** \brief Associations of the Argyris degrees of freedom to subentities of the
     * reference simplex
     *     */
    ...
    public:
      ...
      void interpolate(F const& f, std::vector<C>& out) const
      {
        ...
        for (int i = 0; i < 3; i++, offset += 6)
        {
          ...
          auto&& hessianValue = Hf(x);
          auto&& h = (*averageVertexMeshSize_)[i];

          out[offset ] = f(x);

          out[offset + 1] = squeeze(derivativeValue)[0] * h;
          out[offset + 2] = squeeze(derivativeValue)[1] * h;
        ...
        for (int i = 0; i < 3; i++)
        {
          out[18 + i] = squeeze(df(localMidpoints_[i])).dot(globalNormals_[i]);
        }
    ...
    /** \brief Argyris finite element for simplices, as defined on the reference Element.
     ...
     * For more Details, see <dune/functions/functionspacebases/Argyrisbasis.hh>.
     *
