...
namespace Functions
{
    ...
    public:
      ...
      void bind(const Element &element, const std::bitset<3> &edgeOrientation)
      {
        ...
        for (std::size_t i = 0; i < 3; ++i)
        {
          ...
          edge /= edge.two_norm() * edgeOrientation[i];
  ...
  using MorleyGlobalState = std::vector<bitset<3>>;
  using MorleyLocalState = MorleyGlobalState::value_type;
  ...
  public:
    ...
    void bind(Mapper const& mapper, std::bitset<3> const& data, Element const &e)
    {
      localState_ = data;
  ...
  protected:
     ...
     */
    Impl::MorleyLocalBasis<D, R, dim, reduced> const&referenceLocalBasis() const { return basis_; }
  ...
  private:
    ...
    typename Impl::MorleyLocalBasis<D, R, dim, reduced> basis_;
  ...
  public:
    ...
    using FiniteElement = MorleyLocalFiniteElement<typename GV::ctype, R, GV::dimension, reduced>;


    MorleyNode(Mapper const& m, std::vector<typename GV::ctype> const& data)
      ...
      for (const auto &element : elements(gv)){
        ...
        for (std::size_t i = 0; i < element.subEntities(dim - 1); i++)
        {
          ...
          auto localV1 = refElement.subEntity(i, dim - 1, 1, dim);
            ...
            auto globalV0 = indexSet.subId(element, localV0, dim);
            auto globalV1 = indexSet.subId(element, localV1, dim);
        ...
        data_[elementIndex] = ElementInformation(orientation);
      }

    }

    SubEntityMapper mapper_;
    MorleyGlobalState data_;
