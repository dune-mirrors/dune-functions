  ...
  if constexpr (dim == 1)
  {
    ...
    if (rank == 0)
    {
      ...
      for (unsigned int i = 1; i < elementsPerDim; ++i, shiftDirection *= -1)
      {
        ...
        // randomPermutation<dim>(indices);

        gridFactory.insertElement(GeometryTypes::simplex(dim), indices);
      }
      ...
      for (unsigned int row = 0; row < elementsPerDim; ++row)
      {
        ...
        for (unsigned int col = 0; col < elementsPerDim; ++col)
        {
          ...
          std::vector<unsigned int> indices
              = {row * (elementsPerDim + 1) + col, (row + 1) * (elementsPerDim + 1) + col,
                 (row + 1) * (elementsPerDim + 1) + (col + 1)};
          // randomPermutation<dim>(indices);
          gridFactory.insertElement(GeometryTypes::simplex(dim), indices);
          // upper triangle of rect
