  ...
  namespace TypeTree2 {
    ...
    namespace Detail {
      ...
      class ContainerFactory
      {
        ...
        template<class N>
        using StaticDegreeConcept = decltype((std::integral_constant<std::size_t, std::size_t(N::degree())>{}, true));
      ...
      {
        template<class C>
        static constexpr decltype(auto) accessByTreePathImpl(C&& container)
        {
          return container;
        }

        template<class C, class I0, class... I>
        static constexpr decltype(auto) accessByTreePathImpl(C&& container, I0 i0, I... ii)
        {
          return accessByTreePathImpl(container[i0], ii...);
        }

        template<class C, class... T, std::size_t... i>
        static constexpr decltype(auto) accessByTreePath(C&& container, const TypeTree::HybridTreePath<T...>& tp, std::index_sequence<i...>)
        {
          return accessByTreePathImpl(container, TypeTree::treePathEntry<i>(tp)...);
        }
      ...
      public:
        ...
        template<class... T>
        decltype(auto) operator[](const TypeTree::HybridTreePath<T...>&  path) const
        {
          return accessByTreePath(container_, path, std::index_sequence_for<T...>{});
        }

        template<class... T>
        decltype(auto) operator[](const TypeTree::HybridTreePath<T...>&  path)
        {
          return accessByTreePath(container_, path, std::index_sequence_for<T...>{});
        }
