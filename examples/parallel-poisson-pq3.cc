...
enum class Attributes { owner = 1 };

int main (int argc, char *argv[]) try
{
  // Set up MPI, if available
  auto const& mpiHelper = MPIHelper::instance(argc, argv);

  ///////////////////////////////////
  //   Generate the grid
  ///////////////////////////////////

  auto grid = Dune::YaspGrid<2>({1.0,1.0}, {10,10});
  grid.globalRefine(4);

  auto localGridView = grid.leafGridView();
  using LocalGridView = decltype(localGridView);

  auto globalGridView = Functions::GlobalGridView{localGridView};
  using GlobalGridView = decltype(globalGridView);

  /////////////////////////////////////////////////////////
  //   Choose a finite element space
  /////////////////////////////////////////////////////////

  typedef Functions::LagrangeBasis<LocalGridView,3> LocalFEBasis;
  LocalFEBasis localFeBasis(localGridView);

  typedef Functions::LagrangeBasis<GlobalGridView,3> GlobalFEBasis;
  GlobalFEBasis globalFeBasis(globalGridView);
