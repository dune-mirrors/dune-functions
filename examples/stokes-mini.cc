...
// { local_assembler_signature_end }
{
  ...
  const auto& velocityLocalFiniteElement                   /*@\label{li:stokes_mini_get_velocity_lfe}@*/
          = localView.tree().child(_0,0).finiteElement();
  const auto& pressureLocalFiniteElement
          = localView.tree().child(_1).finiteElement();    /*@\label{li:stokes_mini_get_pressure_lfe}@*/
  // { get_local_fe_end }
  ...
  for (const auto& quadPoint : quad)
  {
    ...
    for (size_t i=0; i<velocityLocalFiniteElement.size(); i++)
      for (size_t j=0; j<velocityLocalFiniteElement.size(); j++ )
        ...
        {
          size_t row = localView.tree().child(_0,k).localIndex(i);                    /*@\label{li:stokes_mini_compute_vv_element_matrix_row}@*/
          size_t col = localView.tree().child(_0,k).localIndex(j);                    /*@\label{li:stokes_mini_compute_vv_element_matrix_column}@*/
          elementMatrix[row][col] += (jacobians[i] * transpose(jacobians[j]))
                                     * quadPoint.weight() * integrationElement;  /*@\label{li:stokes_mini_update_vv_element_matrix}@*/
        }
    ...
    for (size_t i=0; i<velocityLocalFiniteElement.size(); i++)
      for (size_t j=0; j<pressureLocalFiniteElement.size(); j++ )
        for (size_t k=0; k<dim; k++)
        {
          size_t vIndex = localView.tree().child(_0,k).localIndex(i); /*@\label{li:stokes_mini_compute_vp_element_matrix_row}@*/
          size_t pIndex = localView.tree().child(_1).localIndex(j);   /*@\label{li:stokes_mini_compute_vp_element_matrix_column}@*/

          elementMatrix[vIndex][pIndex] +=                    /*@\label{li:stokes_mini_update_vp_element_matrix_a}@*/
                  jacobians[i][0][k] * pressureValues[j]
                  * quadPoint.weight() * integrationElement;
          elementMatrix[pIndex][vIndex] +=
                  jacobians[i][0][k] * pressureValues[j]
                  * quadPoint.weight() * integrationElement;  /*@\label{li:stokes_mini_update_vp_element_matrix_b}@*/
        }
...
// { global_assembler_signature_end }
{
  ...
  for (const auto& element : elements(basis.gridView()))
  {
    ...
    for (size_t i=0; i<elementMatrix.N(); i++)
    {
      ...
      auto row = localView.index(i);                /*@\label{li:stokes_mini_get_global_row_index}@*/

      for (size_t j=0; j<elementMatrix.M(); j++ )
      {
        // The global index of the j-th local degree of freedom of the element 'e'
        auto col = localView.index(j);                /*@\label{li:stokes_mini_get_global_column_index}@*/
        matrixEntry(matrix, row, col) += elementMatrix[i][j];  /*@\label{li:stokes_mini_scatter_matrix_indices}@*/
      }
    }
...
int main (int argc, char *argv[]) try
{
  ...
  rhsBackend.resize(miniBasis);
  rhs = 0;                                 /*@\label{li:stokes_mini_set_rhs_to_zero}@*/
  // { rhs_assembly_end }

  // { matrix_assembly_begin }
  MatrixType stiffnessMatrix;
  assembleStokesMatrix(miniBasis, stiffnessMatrix);   /*@\label{li:stokes_mini_call_to_assemblestokesmatrix}@*/
  // { matrix_assembly_end }
  ...
  isBoundaryBackend.resize(miniBasis);
  isBoundary = false;
  ...
  Functions::forEachBoundaryDOF(
          Functions::subspaceBasis(miniBasis, _0),
          [&] (auto&& index) {
  ...
  {
    return VelocityRange{0.0, (1-x[0])*x[1]*(1-x[1])*4};
  };

  Functions::interpolate(
          Functions::subspaceBasis(miniBasis, _0), rhs,
  ...
  auto localView = miniBasis.localView();
  for(const auto& element : Dune::elements(miniBasis.gridView()))
  {
    localView.bind(element);
    for (size_t i=0; i<localView.size(); ++i)
    {
  ...
  auto velocityFunction
          = Functions::makeDiscreteGlobalBasisFunction<VelocityRange>(
            Functions::subspaceBasis(miniBasis, _0), x);
  auto pressureFunction
          = Functions::makeDiscreteGlobalBasisFunction<PressureRange>(
            Functions::subspaceBasis(miniBasis, _1), x);
...
VTKWriter<GridView> vtkWriter(gridView);
  ...
  vtkWriter.write("stokes-mini-result");
  // { vtk_output_end }
