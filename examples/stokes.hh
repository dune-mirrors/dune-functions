...
#ifndef DUNE_FUNCTIONS_EXAMPLES_STOKES_HH
#define DUNE_FUNCTIONS_EXAMPLES_STOKES_HH
...
namespace Dune::Functions::Examples {
namespace Stokes {

// We show the implementation of two container layouts, the
// BLOCKED layout where the velocity components are stored in separate
// matrices and thus the velocity matrix is blocked, and the
// FLAT layout, where all velocity components are stored in one big
// flat matrix.
enum ContainerLayoutTag { BLOCKED, FLAT };

// Type wrapper to pass around the container layout
template <ContainerLayoutTag layout>
...
template <int dim, ContainerLayoutTag>
struct LinearAlgebraTypes;

template <int dim>
struct LinearAlgebraTypes<dim, BLOCKED>
{
...
template <int dim>
struct LinearAlgebraTypes<dim,FLAT>
{
...
template <class Basis, ContainerLayoutTag layout, class MatrixType>
void setOccupationPattern(const Basis& basis, ContainerLayout<layout>, MatrixType& matrix)
{
  ...
  // the matrix that has an inner flat layout is dynamic in size,
  // thus must be resized to the correct shape.
  if constexpr(layout == FLAT)
    matrix.setSize(2,2);
...
decltype(auto) matrixEntry(ContainerLayout<BLOCKED>,
...
template<class Matrix, class MultiIndex>
decltype(auto) matrixEntry(ContainerLayout<FLAT>,
        Matrix& matrix, const MultiIndex& row, const MultiIndex& col)
{
...
template <class Basis, ContainerLayoutTag tag, class MatrixType>
void assembleStokesMatrix(const Basis& basis, ContainerLayout<tag> layout, MatrixType& matrix)
// { global_assembler_signature_end }
{
  // { setup_matrix_pattern_begin }
  // Set matrix size and occupation pattern
  setOccupationPattern(basis, layout, matrix);
  ...
  for (const auto& element : elements(basis.gridView()))
  {
    ...
    for (size_t i=0; i<elementMatrix.N(); i++)
    {
      ...
      for (size_t j=0; j<elementMatrix.M(); j++ )
      {
        ...
        matrixEntry(layout, matrix, row, col) += elementMatrix[i][j];  /*@\label{li:stokes_taylorhood_scatter_matrix_indices}@*/
      }
    }
...
template <ContainerLayoutTag tag, class Basis>
void run (std::string name, const Basis& stokesBasis, ContainerLayout<tag> layout = {})
{
  using GridView = typename Basis::GridView;
  auto const& gridView = stokesBasis.gridView();

  // The dimension of the grid
  static constexpr int dim = GridView::dimension;

  using VectorType = typename LinearAlgebraTypes<dim,tag>::VectorType;
  using BitVectorType = typename LinearAlgebraTypes<dim,tag>::BitVectorType;
  using MatrixType = typename LinearAlgebraTypes<dim,tag>::MatrixType;
  ...
  rhsBackend.resize(stokesBasis);
  rhs = 0;                                 /*@\label{li:stokes_crouzeix_raviart_set_rhs_to_zero}@*/
  // { rhs_assembly_end }

  // { matrix_assembly_begin }
  MatrixType stiffnessMatrix;
  assembleStokesMatrix(stokesBasis, layout, stiffnessMatrix);   /*@\label{li:stokes_crouzeix_raviart_call_to_assemblestokesmatrix}@*/
  // { matrix_assembly_end }
  ...
  isBoundaryBackend.resize(stokesBasis);
  isBoundary = false;
  ...
  Functions::forEachBoundaryDOF(
          Functions::subspaceBasis(stokesBasis, _0),
          [&] (auto&& index) {
  ...
  using Coordinate = typename GridView::Codim<0> ::Geometry::GlobalCoordinate;
  ...
  Functions::interpolate(
          Functions::subspaceBasis(stokesBasis, _0), rhs,
  ...
  auto localView = stokesBasis.localView();
  for(const auto& element : Dune::elements(stokesBasis.gridView()))
  {
    localView.bind(element);
    for (size_t i=0; i<localView.size(); ++i)
    {
      ...
      if (isBoundaryBackend[row])
        ...
        {
          auto col = localView.index(j);
          matrixEntry(layout, stiffnessMatrix, row, col) = (i==j) ? 1 : 0;
        }
    }
  }
  ...
  auto velocityFunction
          = Functions::makeDiscreteGlobalBasisFunction<VelocityRange>(
            Functions::subspaceBasis(stokesBasis, _0), x);
  auto pressureFunction
          = Functions::makeDiscreteGlobalBasisFunction<PressureRange>(
            Functions::subspaceBasis(stokesBasis, _1), x);
  ...
  vtkWriter.write("stokes-"+name+"-result");
  // { vtk_output_end }

}

} // end namespace Stokes
} // end namespace Dune::Functions::Examples

#endif // DUNE_FUNCTIONS_EXAMPLES_STOKES_HH
